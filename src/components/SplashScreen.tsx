      ...
      <div className="text-center">
        ...
        <div className={`transition-all duration-1000 ${isAnimating ? 'scale-100 opacity-100' : 'scale-110 opacity-90'}`}>
          <div className="relative mb-8">
            <div className="w-32 h-32 mx-auto bg-transparent flex items-center justify-center transform transition-transform duration-1000 hover:scale-105">
              <img 
                src="/log copy.png" 
                alt="Vayara Logo" 
                className={`w-24 h-24 object-contain transition-all duration-1000 ${isAnimating ? 'animate-pulse' : ''}`}
              />
            </div>
            <div className={`absolute -inset-4 bg-gradient-to-r from-pink-300 to-rose-300 rounded-full opacity-20 transition-all duration-1000 ${isAnimating ? 'animate-ping' : ''}`}></div>
          </div>
        </div>
